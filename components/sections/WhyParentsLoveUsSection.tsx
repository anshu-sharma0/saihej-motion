        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {reasons.map((reason, idx) => {
            ...
            return (
              <div
                key={reason.title}
                className="group relative flex flex-col rounded-3xl bg-white p-5 border-2 border-zinc-200/80 shadow-md transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:border-[#FF4D4D]"
              >
                {/* Top Badge */}
                ...
                <div className="flex items-center mb-2 gap-4">
                  {/* Icon wrapper */}
                  <div
                    className={`flex h-14 w-14 items-center justify-center rounded-2xl ${reason.accent} transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}
                  >
